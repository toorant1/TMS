...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        if ($currentTicketId !== $row['Ticket ID']) {
            ...
            echo '<td rowspan="1">' . (!empty($row['Ticket Date']) ? htmlspecialchars(date('d-M-Y', strtotime($row['Ticket Date']))) : 'N/A') . '</td>';
        ...
        echo '<td>' . (!empty($row['Bill Date']) ? htmlspecialchars(date('d-M-Y', strtotime($row['Bill Date']))) : '') . '</td>';
        ...
        echo (!empty($row['Due Date']) ? htmlspecialchars(date('d-M-Y', strtotime($row['Due Date']))) : '') . '</td>';
