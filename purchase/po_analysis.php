...
$sql = "SELECT po.id, po.po_number, po.po_date, po.quotation_no, 
               ...
               po.po_status,  -- ✅ Ensure po_status is selected
               pos.status_name AS status  -- ✅ Fetch status_name from purchase_order_status
        FROM purchase_orders po
        INNER JOIN purchase_order_status pos ON po.po_status = pos.id  -- ✅ Proper INNER JOIN
...
// Fetch Material Price Analysis (Last 10 POs per Material)
/*$sql_price_analysis = "
    SELECT pom.material_id, mm.name AS material_name, acc.account_name AS supplier, pom.unit_price, po.po_number, po.po_date, po.id, po.token, mc.company_name as company
    FROM purchase_order_materials pom
    ...
    ORDER BY pom.material_id, po.po_date DESC
    LIMIT 10";*/
    ...
    <div class="d-flex">
        ...
        <div class="content w-100">
            <div class="container mt-4">
                <h4 class="text-center mb-3">Purchase Order Analysis & Approval Management</h4>
            ...
            <?= isset($po['po_status']) ?
                ($po['po_status'] == 2 ? 'bg-warning' : ($po['po_status'] == 3 ? 'bg-success' : ($po['po_status'] == 4 ? 'bg-danger' : ($po['po_status'] == 5 ? 'bg-secondary' : ($po['po_status'] == 6 ? 'bg-warning text-dark' : 'bg-dark')))))
                : 'bg-dark' ?>">
                                        <?= isset($po['status']) ? "PO Status: " . htmlspecialchars($po['status']) : "Unknown Status" ?>
                                    </span>
                                </div>
                            </div>
                                    ...
                                    <?php if ($remarks_result->num_rows > 0): ?>
                                        <?php while ($remark = $remarks_result->fetch_assoc()): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="badge 
                                        <?= ($remark['status'] == 3) ? 'bg-success' : (($remark['status'] == 4) ? 'bg-danger' : (($remark['status'] == 5) ? 'bg-secondary' : (($remark['status'] == 6) ? 'bg-warning' : 'bg-dark'))); ?>">
                                                        <?= ($remark['status'] == 3) ? 'Approved' : (($remark['status'] == 4) ? 'Rejected' : (($remark['status'] == 5) ? 'Cancelled' : (($remark['status'] == 6) ? 'Revision Required' : 'Unknown'))); ?>
                                                    </span>
                                                </td>
                ...
                <div class="card shadow-sm mt-4">
                    <div class="card-header bg-secondary text-white">
                        <i class="bi bi-check-circle"></i> Purchase Order - Decision
                    </div>
                    <div class="card-body">
                        <form id="updateStatusForm">
                            <input type="hidden" name="po_token" value="<?= htmlspecialchars($po_token); ?>">
                            <input type="hidden" name="approver_id" value="<?= htmlspecialchars($master_userid); ?>">

                            <div class="row g-3">
                                <!-- Status Dropdown -->
                                <div class="col-md-4">
                                    <label class="form-label fw-bold">Select PO Status</label>
                                    <select class="form-select" name="po_status" required>
                                        <option value="">-- Select Status --</option>
                                        <option value="3" <?= $po['po_status'] == 3 ? 'selected' : ''; ?>>Approved</option>
                                        <option value="4" <?= $po['po_status'] == 4 ? 'selected' : ''; ?>>Rejected</option>
                                        <option value="5" <?= $po['po_status'] == 5 ? 'selected' : ''; ?>>Cancelled</option>
                                        <option value="6" <?= $po['po_status'] == 5 ? 'selected' : ''; ?>>Revision Required</option>
                                    </select>
                                </div>

                                <!-- Remark Textbox -->
                                <div class="col-md-8">
                                    <label class="form-label fw-bold">Approver Remark</label>
                                    <textarea class="form-control" name="approver_remark" rows="3" placeholder="Enter remarks here..." required></textarea>
                                </div>
                            </div>
                ...
                <!-- Success Message -->
                <div id="updateSuccessMessage" class="alert alert-success d-none mt-3 text-center">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            document.getElementById("updateStatusForm").addEventListener("submit", function(event) {
                ...
                fetch("update_po_status_approvals.php", {
                        method: "POST",
                        body: formData
                    })
