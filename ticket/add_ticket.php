...
<body>
    ...
    <div class="modal fade" id="addContactModal" tabindex="-1" aria-labelledby="addContactModalLabel" aria-hidden="true">
        ...
        <input type="hidden" id="master-user-id" value="<?php echo htmlspecialchars($_SESSION['master_userid']); ?>">

<div id="ticket-details-section" class="mt-4">
    ...
    <script>
        $(document).ready(function() {
            ...
            function fetchTicketHistory(accountId) {
    const masterUserId = $('#master-user-id').val(); // Fetch master_user_id from hidden input

    // Log the parameters to check their values
    console.log('Master User ID:', masterUserId);
    console.log('Account ID:', accountId);

    if (!accountId || !masterUserId) {
        alert('Missing required parameters: Account ID or Master User ID.');
        return;
    }
    ...
    $.ajax({
        url: '../api/ticket/ticket_history.php',
        type: 'POST',
        dataType: 'json',
        data: {
        ...
        success: function (response) {
            if (response.status === 'success') {
                ...
                $('#ticket-details-content').html(''); // Clear previous content
                $('#ticket-details-content').load('../api/ticket/ticket_details.php', {
                    data: JSON.stringify(response.data),
                });
            } else {
                alert(response.message || 'No ticket details found.');
                $('#ticket-details-content').empty();
            }
        },
