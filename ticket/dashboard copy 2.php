...
$query = "
    ...
    ORDER BY mt.ticket_id DESC, mt.ticket_date DESC
";
                ...
                <tbody>
                    <?php if ($result->num_rows > 0): ?>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr>
                                <td>
                                    <a href="ticket_operation.php?ticket_id=<?= urlencode($row['Ticket ID']); ?>&token=<?= urlencode($row['Ticket Token']); ?>"
                                        class="text-primary" target="_blank">
                                        <?= htmlspecialchars($row['Ticket ID']); ?>
                                    </a><br>
                                    <?= htmlspecialchars($row['Ticket Date']); ?>
                                </td>
                                <td>
                                    <a href="account_tickets.php?account_id=<?= urlencode($row['Account ID']); ?>"
                                        class="text-primary" target="_blank">
